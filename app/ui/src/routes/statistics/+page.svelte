...
<div class="space-y-6">
	...
	<div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
		<div>
			...
			<p class="mt-1 text-sm text-gray-400">Detailed insights into your relay activity</p>
		</div>
	...
	{:else}
		...
		{#if EventsOverTimeChart}
			<EventsOverTimeChart data={eventsOverTime.data} total={eventsOverTime.total} />
		{:else}
			<div class="rounded-lg bg-gray-800 p-6">
				<h3 class="text-lg font-semibold text-white">Events Over Time</h3>
				<p class="text-gray-400">{eventsOverTime.total} total</p>
			</div>
		{/if}
		...
		<div class="grid gap-6 lg:grid-cols-2">
			{#if EventsByKindChart}
				<EventsByKindChart kinds={eventsByKind.kinds} total={eventsByKind.total} />
			{:else}
				<div class="rounded-lg bg-gray-800 p-6">
					<h3 class="text-lg font-semibold text-white">Events by Kind</h3>
					<p class="text-gray-400">{eventsByKind.total} total</p>
				</div>
			{/if}

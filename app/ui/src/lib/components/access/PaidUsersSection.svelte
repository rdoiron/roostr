	...
	<div class="flex items-center justify-between mb-4">
		<div>
			<h2 class="text-lg font-semibold text-gray-900 dark:text-gray-100">
				Paid Users
				<span class="ml-2 text-sm font-normal text-gray-500 dark:text-gray-400">({total})</span>
			</h2>
			<p class="text-sm text-gray-500 dark:text-gray-400">Users who purchased access via Lightning payment</p>
		</div>
	</div>
	...
	<div class="flex items-center justify-between mb-4 pb-4 border-b dark:border-gray-700">
		...
		<div class="relative">
			<input
				...
				class="w-64 pl-10 pr-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 text-sm focus:ring-2 focus:ring-purple-500 focus:border-transparent"
			/>
	...
	{#if filteredUsers().length === 0}
		<div class="text-center py-8">
			...
			{#if search || filter !== 'all'}
				<p class="text-gray-500 dark:text-gray-400">No users match your filters</p>
				<button
					type="button"
					onclick={() => { search = ''; filter = 'all'; }}
					class="mt-2 text-sm text-purple-600 dark:text-purple-400 hover:text-purple-500"
				>
					Clear filters
				</button>
			{:else}
				<p class="text-gray-500 dark:text-gray-400">No paid users yet</p>
			...
			<div class="flex items-center justify-between mt-4 pt-4 border-t dark:border-gray-700">
				...
				<div class="flex items-center space-x-2">
					<button
						...
						onclick={() => page--}
						disabled={page === 0}
						class="px-3 py-1.5 text-sm font-medium text-gray-700 dark:text-gray-200 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-600 disabled:opacity-50 disabled:cursor-not-allowed"
					>
						Previous
					</button>
					<span class="text-sm text-gray-500 dark:text-gray-400">Page {page + 1} of {totalPages}</span>
					<button
						type="button"
						onclick={() => page++}
	...
	>
		<div class="w-full max-w-md rounded-lg bg-white dark:bg-gray-800 p-6 shadow-xl">
			<div class="flex items-center space-x-3 mb-4">
				...
				<div>
					<h3 id="revoke-modal-title" class="text-lg font-semibold text-gray-900 dark:text-gray-100">Revoke Access</h3>

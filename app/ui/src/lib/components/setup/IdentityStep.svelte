...
<div>
	<h2 class="text-2xl font-bold text-gray-900 mb-2">Your Identity</h2>
		...
		{#if validationResult.valid}
			<div class="p-3 bg-green-50 border border-green-200 rounded-lg mb-4">
				<div class="flex items-start space-x-2">
					...
					<div class="text-sm">
						<p class="font-medium text-green-800">Valid {validationResult.source === 'nip05' ? 'NIP-05' : 'pubkey'}</p>
						<p class="text-green-700 font-mono text-xs break-all mt-1">
							{validationResult.npub}
						</p>
						{#if validationResult.nip05_name}
							<p class="text-green-600 mt-1">Resolved from: {validationResult.nip05_name}</p>
						{/if}
					</div>
				</div>
			</div>
		{:else}
			<div class="p-3 bg-red-50 border border-red-200 rounded-lg mb-4">
				<div class="flex items-start space-x-2">
					...
					<div class="text-sm">
						<p class="font-medium text-red-800">Invalid identity</p>
						<p class="text-red-700 mt-1">{validationResult.error || 'Please enter a valid npub, hex pubkey, or NIP-05 identifier.'}</p>
					</div>
				</div>
			</div>
		{/if}
